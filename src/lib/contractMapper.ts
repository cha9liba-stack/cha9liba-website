...
export function mapFirebaseToContract(id: string, raw: Record<string, any>): Contract {
  ...
  return {
    ...
    depot:           str(raw["المبلغ المدفوع من قبل العميل"] ?? raw["ضمان الايداع"] ?? raw["Dépôt garantie"] ?? raw["depot"] ?? raw["avance"] ?? ""),
    depotGarantie:   str(raw["ضمان الايداع"] ?? raw["Dépôt garantie"]),
    prep:            str(raw["prep"]),
    total:           str(raw["الجملة"] ?? raw["Total"]),
    somme:           str(raw["المجموع"] ?? raw["Somme"]),
    resteAPayer:     str(raw["المبلغ المتبقي"] ?? raw["resteAPayer"] ?? raw["Reste à payer"] ?? raw["reste"] ?? "0"),
    // Other
